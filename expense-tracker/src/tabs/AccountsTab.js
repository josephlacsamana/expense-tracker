...
import { Plus, Trash2, Edit3, X, ChevronDown, ChevronRight, AlertTriangle, Wallet, Coins, Clock, Bell, RefreshCw, ArrowLeft, PiggyBank, Target, TrendingDown, DollarSign, Repeat } from "lucide-react";
import { useApp } from "../AppContext";
import { aIcons, dIcons, DEBT_TYPES, INCOME_SOURCES, CRYPTO_COINS, fmt, fmtS, td, uid, pld } from "../constants";

export default function AccountsTab() {
  const { exp, accts, budgets, genBudget, cats, rec, debts, dPays, acctHist, savGoals, income, recIncome, catColors, svE, svA, svB, svCats, svGB, svR, svD, svDP, svAH, svSG, svI, svRI, tst, user, users, theme, isDesktop, T, cardS, pillS, inpS, btnP, btnG, mOvS, mInS, cryptoPrices, cryptoLastUpdated, fetchCryptoPrices } = useApp();
  ...
  const [sgSavedIn, setSgSavedIn] = useState("native"); // "native" = enter in BTC/ETH/etc, "usd" = enter in USD (auto-convert)
  // Income states
  ...
  const [incSub, setIncSub] = useState("list"); // "list" | "recurring"
      ...
      {sgForm && <div style={mOvS}><div style={mInS}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button onClick={() => {
            ...
            // If crypto goal with "saved in USD" toggle, convert USD to crypto amount
      ...
      {sgAdd && (() => { const addGoal = savGoals.find(g => g.id === sgAdd); const addCur = addGoal?.currency || "PHP"; const addIsPhp = addCur === "PHP"; const addIsUsd = addCur === "USD"; const addIsCr = !addIsPhp && !addIsUsd; const addCp = cryptoPrices[addCur]; return <div style={mOvS}><div style={mInS}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button id="sgAddBtn" onClick={() => {
            ...
            if (!g) return;
            // If entering in USD for a crypto goal, convert to crypto amount
