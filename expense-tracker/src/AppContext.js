...
export function AppProvider({ children, user, householdId, householdRole, profile, household, theme }) {
  ...
  const svDP = async (d, opts) => { setDPays(d); try { if (sbReady) { if (opts?.upsert) await sb.upsertDebtPayment(opts.upsert, householdId); else if (opts?.upsertMany) await sb.upsertDebtPayments(opts.upsertMany, householdId); } else await localStore.set("debtPayments", JSON.stringify(d)); } catch {} };
