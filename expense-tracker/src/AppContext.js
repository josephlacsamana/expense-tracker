...
export function AppProvider({ children, user, householdId, householdRole, profile, household, theme }) {
  ...
  const callAI = async (m, s, ret = 2) => {
    for (let i = 0; i <= ret; i++) {
      try {
        const r = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-6-20250520", max_tokens: 1000, system: s, messages: m }) });
